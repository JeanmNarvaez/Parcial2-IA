...
class C45:
  ...
  def seleccionar_atributo(self, X, Y, antecedente):
      ...
      for i, atributo in enumerate(X.T):
          if i not in antecedente:
              ganancia_ratio.append(self.calcular_ganancia_ratio(atributo, Y))
          else:
              ganancia_ratio.append(float("-inf"))
      return ganancia_ratio.index(max(ganancia_ratio))

  def calcular_ganancia_ratio(self, atributo, Y):
      ganancia = self.calcular_ganancia(atributo, Y)
      split_info = 0

      for valor in np.unique(atributo):
          proporcion = np.sum(atributo == valor) / len(atributo)
          split_info -= proporcion * np.log2(proporcion)

      ganancia_ratio = ganancia / split_info if split_info != 0 else 0
      return ganancia_ratio
  ...
  def calcular_ganancia(self, atributo, Y):
      ...
      ganancia = (self.calcular_entropia(Y) - entropia_atributos) / split_info if split_info != 0 else 0
      return ganancia
...
atributo_ejemplo = X[:, 0]  # Replace 0 with the index of the attribute you want to calculate gain ratio for
ganancia_ratio = arbol.calcular_ganancia_ratio(atributo_ejemplo, Y)
print(f'Proporción de ganancia para el atributo: {ganancia_ratio}')
